// This the professor's code from the lecture notes which was edited
...
void KruskalMST(struct Graph* graph)
{
    ...
    opfile << "The total number of edges:" << e << endl;
    for (i = 0; i < e; ++i) {
        opfile << result[i].src << " "  << result[i].dest << endl;
    }
...
Data *ReadData(string filename){
    ...
    while(getline(ipfile, str)){
        ...
        if(V == 0) {
            ...
            for (int i=0; i<V; i++)
                coordinates[i] = (int *)malloc(3 * sizeof(int));
        }
        else {
            int j = 0;
            while (token1 != NULL) {
...
int getDistance(int i, int j, int** coordinates){
    ...
    int x2 = coordinates[j][1];
    int y2 = coordinates[j][2];
...
int main()
{
    ...
    int V = data->V;  // Number of vertices in graph
    int E = V * V;  // Number of edges in graph
    struct Graph* graph = createGraph(V, E);
    int index = 0;
    for(int i = 0; i< V ; i++){
        for(int j = 0; j< V; j++){
            graph->edge[index].src = i;
            graph->edge[index].dest = j;
            graph->edge[index].weight = getDistance(i, j, data->coordinates);
            index++;
            //cout << i << " " << j << " " << graph->edge[i + j].weight << endl;
        }
    }
